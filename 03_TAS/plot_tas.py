...
tas_data = specData('./', 4)
ref_time_idx = np.argmin(abs(tas_data.delaytime - ref_time))
...
im = plotTasAxis(ax, tas_data.avespec-tas_data.avespec[ref_time_idx], tas_data.energies, tas_data.delaytime, 
        vmax=(tas_data.avespec[-1] - tas_data.avespec[ref_time_idx]).max())
fig.colorbar(im, ax=ax)
ax.axvline(pumped_energy, c='k', linestyle='--')
ax.set_xlim(1.0, 8.0)
ax.set_ylim(50, 10)
